...
show_pmenu () {
    menu="$(rofi -sep "|" -dmenu -i -p "$DEV_NAME" -location $LOCATION -yoffset $YOFFSET -xoffset $XOFFSET -theme $THEME -width $WIDTH -hide-scrollbar -line-padding 1 -padding 20 -lines 1<<<"Par Device")"
                case "$menu" in
...
do
    ...
    case $c in
        d) show_devices ;;
        i) DEV_ID="$(kdeconnect-cli -a --id-only)" ;;
        n) DEV_NAME=$OPTARG ;;
